...
/*! \page advanced Дополнительные сведения
\tableofcontents
<pre>
...
<pre>
Определяет переменные стандартных типов и объекты OpenAL, которые необходимы для
корректной работы методов, в т. ч. производных классов, воспроизведения звуков.
...
int Reductor::play(Helicopter h, SOUNDREAD sr)
{
	...
	if (h.modelName == "mi_28")
	{
		if (sr.reduktor_gl_obor > h.redTurnoverAvt - 2)
		{
			...
			if (abs(velocityX) < 60)
			{
				pinkNoiseGain = interpolation(0, -60, 50, -14, 60, -6, abs(velocityX));
			}
			else
			{
				pinkNoiseGain = interpolation(60, -6, 70, -3, 80, 0, abs(velocityX));
			}
